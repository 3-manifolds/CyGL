...
One of the most egregious design flaws in OpenGL, or at least in most OpenGL
implementations, is that when the library is first loaded it is in a state
where its pointer to the current GL context is NULL.  Worse, no GL functions
test that pointer.  Instead, acall to any GL function when the library is
in its initial state will produce a segfault.  It would seem to be simple
enough to initialize the library with a default context which returns an
error for any GL function call.  But, it is what it is.

The gl_context_exists function works by installing a SIGSEGV handler and then
calling GLGetString.  If the handler gets called then the function returns
...
#include <signal.h>
#include <setjmp.h>
static sig_t saved_handler = SIG_DFL;
